...
(async () => {
  try {
    ...
    */
    ...
    $.logger.info(
      '[DTLS Client] DTLS client is running and ready to accept secure connections'
    );
    $.logger.info(
      `[DTLS Client] Listening on port ${localPort} for DTLS connections`
    );

    // Display information about testing
    $.logger.info('[DTLS Client] Testing instructions:');
    $.logger.info(
      '[DTLS Client] 1. Use a DTLS-enabled CoAP client to connect to this server'
    );
    $.logger.info('[DTLS Client] 2. Example command with libcoap:');
    $.logger.info(
      `[DTLS Client]    coap-client -m GET -k q2w3e4r5t6 coaps://localhost:${localPort}/3/0/0`
    );
  ...
  updateTimer = setInterval(async () => {
    if (!$.client.registered) {
      try {
        ...
        $.logger.error(
          `[DTLS Client] Re-registration failed: ${regErr.message}`
        );
      }
      return;
    } else {
      try {
        ...
        $.logger.error(
          `[DTLS Client] Lost connection to server during update: ${err.message}`
        );
        $.client.registered = false;
