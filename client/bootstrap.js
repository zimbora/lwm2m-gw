...
function requestBootstrap(
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('response', (res) => {
      ...
      if (res.code !== '2.04') {
        $.logger.error(
          `[Bootstrap Client] Bootstrap request failed: ${res.code}`
        );
        return reject(
          new Error(`Bootstrap failed: ${res.code} ${res.payload.toString()}`)
        );
      }
    ...
    req.on('error', (err) => {
      clearInterval(timeout);
      $.logger.error(
        `[Bootstrap Client] Bootstrap request error: ${err.message}`
      );
      reject(err);
    });
...
function sendBootstrapFinish(
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('response', (res) => {
      ...
      if (res.code !== '2.04') {
        $.logger.error(
          `[Bootstrap Client] Bootstrap finish failed: ${res.code}`
        );
        return reject(
          new Error(
            `Bootstrap finish failed: ${res.code} ${res.payload.toString()}`
          )
        );
      }
    ...
    req.on('error', (err) => {
      clearInterval(timeout);
      $.logger.error(
        `[Bootstrap Client] Bootstrap finish error: ${err.message}`
      );
      reject(err);
    });
  ...
  return new Promise((resolve, reject) => {
    ...
    const checkProvisioning = setInterval(() => {
      ...
      if ($.client.provisioned) {
        ...
        if (true) {
          ...
          $.logger.warn(
            '[Bootstrap Client] Security and Server are not defined'
          );
    ...
    setTimeout(() => {
      if (!provisioningComplete) {
        ...
        clearInterval(checkProvisioning);
        $.logger.info(
          '[Bootstrap Client] Provisioning completed (timeout fallback)'
        );
        resolve();
      }
    }, 15000);
...
async function performBootstrap(
  ...
  try {
    $.logger.info(
      `[Bootstrap Client] Starting bootstrap sequence for ${endpointName}`
    );
    ...
    $.logger.info(
      `[Bootstrap Client] Bootstrap sequence completed successfully`
    );
    return true;
  } catch (error) {
    $.logger.error(
      `[Bootstrap Client] Bootstrap sequence failed: ${error.message}`
    );
    throw error;
  }
}
